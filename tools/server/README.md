...
| `--models-max-per-device N` | for router server, maximum number of models to load simultaneously per `--device` value (default: 0, 0 = disabled)<br/>(env: LLAMA_ARG_MODELS_MAX_PER_DEVICE) |
...
- **Responses**: `web_search_call` items (`queries`; `action.sources` / `results` when `include` requests them); `url_citation` annotations on `output_text`; streaming emits `response.output_item.added` -> `response.web_search_call.in_progress` -> `response.web_search_call.searching` -> `response.web_search_call.completed` -> `response.output_item.done`, then one `response.output_text.annotation.added` per citation before `response.output_text.done`, followed by the `content_part` / `output_item` done events that carry the final annotations.
...
You may find available preferences in [settings-keys.ts](../ui/src/lib/constants/settings-keys.constants.ts).
