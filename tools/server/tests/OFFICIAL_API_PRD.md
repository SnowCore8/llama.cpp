...
| `prompt_cache_options.ttl` 只收 `30m` | 官方「defaults to `30m`, which is currently the only supported value」 | 其它值 -> 400 `'prompt_cache_options.ttl' must be '30m'`；Anthropic `cache_control.ttl` 的 `5m`/`1h` 改走内部通道 `__prompt_cache_ttl`（**本波；见 DIFF §4.1 原 #3**） |
...
| Anthropic `max_tokens` 必填 | create 文档 `max_tokens: number`（必填，`0` = 只预热缓存）；`count_tokens` body 参数表无该字段 | create 缺失/null -> 400 `'max_tokens' is required`、非整数/负数 -> 400；`count_tokens` 不要求该字段（**本波；见 DIFF §4.1「新」**） |
| Anthropic 非流式错误体 | 官方 `{"type":"error","error":{type,message},"request_id"}` | 401 中间件 / 503 中间件 / `ex_wrapper` 三处按 `is_anthropic_api_path` 改发官方信封（本地 type -> 官方 type/状态码映射，503 -> 529 `overloaded_error`）（**本波；见 DIFF §4.1「新」**） |
...
11. Anthropic 非流式错误信封运行期验证 - **已完成（2026-09-14 本波）**：单模型 9B（8091）+ `POST /v1/messages` 探针逐字节核对 401（无 key / 错 key）、400（body 校验）、200（正常）、529（`overloaded_error`，启动窗口轮询触发）；401/400/529 均为 `{"type":"error","error":{type,message},"request_id":null}` 三键信封，`error.type` 分别 `authentication_error`/`invalid_request_error`/`overloaded_error`，503 -> 529 映射生效。证据 `/tmp/acc-anth-wave/probe-anthropic-envelope.txt`、`probe-anthropic-503.txt`。
...
4. `prompt_cache_options.ttl` 超集 - **已对齐**（外部只收官方 `30m`，另值 400；Anthropic `cache_control.ttl` 的 `5m`/`1h` 改走内部 `__prompt_cache_ttl`；见 DIFF §4.1 原 #3）；
...
26. Anthropic `/v1/messages` 的近似与不生效项（2026-09-14 **二次收束**：首轮合理加深后本波已接线 `thinking{type:"adaptive"}`/`display`、`tools[]`/`tool_use`/`tool_result` 上的 `cache_control`、`service_tier`/`container`/`inference_geo`（校验后丢弃）、响应 `usage.service_tier`/`stop_details`/`container`/`inference_geo`/`cache_creation`/`server_tool_use`、`message_delta.usage` 的 input/cache_*、非流式错误体官方包裹、`max_tokens` 必填（create））；**仍登记**：`tools[].strict`/`input_examples`（接受即忽略）、`usage.service_tier` 恒 `"standard"`、`cache_creation` 恒 0、`inference_geo` 恒 null、`cache_control` 于 `tool_use`/`tool_result` 为消息级近似；见 §5 #14 / DIFF §4 #20 与 §4.1；
...
| 外部 `prompt_cache_options.ttl` 只收 `30m`（`5m`/`1h` -> 400） | 本波（官方接口对齐） | 之前用 `5m`/`1h` 走该字段的客户端现在收 400（Anthropic `cache_control.ttl` 不受影响，走内部通道）；`24h` 仍只属于 `prompt_cache_retention` | 官方「only supported value」；Anthropic 路径改内部通道后仍支持 5m/1h；独立改动，可单独回退 |
...
| Anthropic `max_tokens` 由「缺省 4096」改为必填 | 本波（官方接口对齐） | 之前省略 `max_tokens` 的请求被按 4096 受理，现为 400 `'max_tokens' is required`（`/v1/messages/count_tokens` 不要求该字段，计数路径不受影响） | 官方 create 文档该字段必填；独立改动，可单独回退 |
| Anthropic 非流式错误体改官方信封 | 本波（官方接口对齐） | `/v1/messages`（含 `count_tokens`）的错误体由 OpenAI 四字段包裹改为 `{"type":"error","error":{type,message},"request_id"}`，503 的 HTTP 状态码按官方口径为 529 | 官方非流式错误体形状；其余端点信封不变；独立改动，可单独回退 |
