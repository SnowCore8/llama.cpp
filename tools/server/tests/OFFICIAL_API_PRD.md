...
| 15 | 官方接口对齐收束波（obfuscation / ttl / embeddings / Anthropic 全量接线 + 两处运行期补验） | 2026-09-14 | §4 中可在本地落地项的收束：Responses SSE `obfuscation` 收窄到 `*.delta`（Chat/Completions 不变）、外部 `prompt_cache_options.ttl` 只收 `30m`（Anthropic `cache_control.ttl` 走内部通道 `__prompt_cache_ttl`）、embeddings `encoding_format` 官方 message 400、Anthropic 非流式错误体改官方信封（401/503 -> 529）+ `max_tokens` 必填（`count_tokens` 豁免）+ `thinking.adaptive`/`display` + `cache_control`（tools/tool_use/tool_result）+ `service_tier`/`container`/`inference_geo` 接线 + 响应侧 `usage` 细节与 `message_delta.usage` 累计；`prompt_cache_diagnostics` 补 `context_compacted`；套件行更新（ttl 行改名、`checks_ws.py` 新增未知 model 行） | **已提交**：`ba9b9bbef`（diagnostics）+ `02fe763e4`（obfuscation）+ `e93090e88`（ttl）+ `77887e312`（Anthropic）+ `c219603c9`（embeddings）+ `afc6cd4e8`（WS 套件行）+ `891a13277`（偏差注释）；文档随本次提交 | **acceptance 三套件 530 = 506 PASS / 24 SKIP / 0 FAIL**（responses 378 = 356/22、chat 143 = 142/1、local_durability 9 = 8/1，exit=0；V100 + Qwen3.5-9B-Q4_K_M、单模型 8091；证据 `/tmp/acc-anth-wave/reports/`）；与上一基线 `/tmp/best-of-slots/reports-ttlfix`（去重键 521）对拍：519 共有键 **0 状态变化**、键差异仅 2 对（ttl 行改名，均 PASS）；`--only ws` 41 = 39 PASS / 2 SKIP / 0 FAIL；单测 `unit/test_compat_anthropic.py` + `unit/test_security.py` = **74 passed / 4 deselected（61.01 s，exit=0）**；运行期探针（Anthropic 401/400/200/529 信封、WS 未知 model 帧、echo+logprobs × checkpoint/KV 4/4 PASS）见 §8.1 |

| 16 | Anthropic 面移除（维护成本裁定） | 2026-09-14 | 彻底移除 Anthropic Messages 面：`/v1/messages` 与 `/v1/messages/count_tokens` 两端点、`server_chat_convert_anthropic_to_oai`（含 `normalize_anthropic_billing_header`）、`to_json_anthropic` / `to_json_anthropic_stream`、Anthropic 错误信封与 type 词表（`format_anthropic_error*` / `anthropic_error_status_from_body` / `is_anthropic_api_path` / `format_anthropic_sse`）、`TASK_RESPONSE_TYPE_ANTHROPIC` 与 `anthropic_*` 参数、`X-Api-Key` 认证别名、`__prompt_cache_ttl` 内部通道，以及整个 `unit/test_compat_anthropic.py`；README / README-dev / SCOPE / DIFF / API_SURFACE_SPLIT（由三面收窄为两面）同步改写 | **已提交**：`925507197`（代码）+ `94b600fc3`（单测）；文档随本次提交 | **acceptance 三套件 531 = 507 PASS / 24 SKIP / 0 FAIL**（responses 379 = 357/22、chat 143 = 142/1、local_durability 9 = 8/1，exit=0；V100 + Qwen3.5-9B-Q4_K_M、单模型 8091；证据 `/tmp/acc-noanth/`）；与 S1 基线 `/tmp/acc-split-s1/reports` 对拍 **524 个共有键逐行 0 状态变化（IDENTICAL）**；离线单测 `unit/test_security.py` = **28 passed**（`/tmp/acc-noanth/unit-noanth2.log`）；运行期 `POST /v1/messages` 与 `/v1/messages/count_tokens` -> **404**、`/v1/chat/completions` -> 200。`/root/anthropic-docs` 语料自带面移除起不再被引用 |
...
| `prompt_cache_options.ttl` 只收 `30m` | 官方「defaults to `30m`, which is currently the only supported value」 | 其它值 -> 400 `'prompt_cache_options.ttl' must be '30m'`（**本波；见 DIFF §4.1 原 #3**） |
| Responses SSE `obfuscation` 范围 | 请求侧文档「on streaming **delta events**」；事件 schema 仅 `response.shell_call_command.delta` 声明该字段 | 只在 `*.delta` 事件注入（Chat/Completions 的整块注入按官方 chunk schema 保持不变）（**本波；见 DIFF §4.1 原 #1**） |
| Completions `best_of` 与 `n` | 散文「must be greater than `n`」，但 schema 默认 `best_of: 1` / `n` 默认 1（等值合法） | `best_of < n` -> 400；`best_of == n` 合法（**本波口径修正；见 DIFF §4.1 原 #8**） |
| Embeddings `encoding_format` 非 string 的 message | 400 + 官方 message「The format to return the embeddings in. Can be either float or base64」（`openai-docs/.../embeddings/methods/create.md:51`） | 非 string 与非法字符串都返回该 message（去 markdown 链接）+ 400，不再透传 nlohmann 原文（**本波；见 DIFF §4.1 原 #11**） |
...
| 同口径全量复核，官方接口对齐收束波（ctx=262144；代码 `ba9b9bbef` + `02fe763e4` + `e93090e88` + `77887e312` + `c219603c9` + `afc6cd4e8` + `891a13277`） | 2026-09-14 本波（V100 + Qwen3.5-9B-Q4_K_M、单模型 8091） | **已完成**：530 = 506 PASS / 24 SKIP / 0 FAIL（responses 378 = 356/22、chat 143 = 142/1、local_durability 9 = 8/1，exit=0）；与上一基线 `/tmp/best-of-slots/reports-ttlfix`（suite+name 去重键 521）对拍：519 共有键 **0 状态变化**、键差异仅 2 对（ttl 行改名 `prompt_cache_options_ttl` -> `prompt_cache_options_ttl_only_30m`，两侧均 PASS）；另单独跑 `--only ws` 41 = 39 PASS / 2 SKIP / 0 FAIL（新增未知 model 行转 PASS） | `/tmp/acc-anth-wave/reports/`、`/tmp/acc-anth-wave/reports-ws-only.json` |
...
11. ~~Anthropic 非流式错误信封运行期验证~~ - **作废（2026-09-14，该面已移除）**：`/v1/messages` 与 `/v1/messages/count_tokens` 已删除，原 401/400/200/529 信封验证不再适用；移除后的端点探针（两端点 -> 404、Chat -> 200）见 §5 #16。
12. Responses WebSocket 未知 model - **已完成（2026-09-14 本波，已套件化）**：原为 `/tmp` 一次性探针，现改为永久回归行（`checks_ws.py` `unknown_model()`，`_RUNNABLE_NAMES` + `guarded(...)` 注册）；单跑 `--only ws` **41 = 39 PASS / 2 SKIP / 0 FAIL**，新行 PASS（服务端回官方格式未知-model 错误帧）。证据 `/tmp/acc-anth-wave/reports-ws-only.json`（探针原件 `probe_ws_unknown_model.py` / `probe-ws-unknown-model.{txt,json}` 保留为过程证据）。
13. echo+logprobs 与 checkpoint/KV 互不污染 - **已完成（2026-09-14 本波）**：4/4 PASS。代码侧依据 `server-context.cpp:3507` 的 `cache_prompt && !need_prompt_logits` 门给 echo+logprobs 请求 `n_past = 0`（既不消费 checkpoint 也不继承脏 KV）；运行期在同一槽位刚处理完同 10391-token prompt 后，echo+logprobs 请求 `cached=0`、行数恰为 `prompt_tokens + completion_tokens`、`text_offset` 从 0 连续到 38641、`join == text`、第 0 行双 `null`；紧随的普通请求 `cached=8353`，证明 KV 复用/checkpoint 恢复未被破坏。证据 `/tmp/acc-anth-wave/probe_echo_logprobs_ckpt.py`、`probe-echo-logprobs-ckpt.{txt,json}`。
14. 单测回归（2026-09-14） - **已完成**：`unit/test_security.py` = **28 passed**（`unshare -n`，零 HF 下载；evidence `/tmp/acc-noanth/unit-noanth2.log`，exit=0）。原 Anthropic 单测腿（`unit/test_compat_anthropic.py`，44 条）随该面移除删除（见 §5 #16），原「74 passed / 4 deselected」口径作废。

来源：DIFF §6 注（「9B 全量新基线待重跑回填」）+ 2026-09-14 用户口径指令 + 本轮 §5 #9 / §5 #10 / §5 #11 / §5 #12 / §5 #13 / §5 #14 / §5 #15。

**本轮已跑运行期探针**（`/tmp/oai-probe/verify_b2a_echo_logprobs.py`，非流式 `/v1/completions` echo+logprobs 36/36 PASS；V100 + Qwen3.5-9B-Q4_K_M，证据 `/tmp/oai-probe/b2a-verify/`）；**acceptance 三套件已跑全绿**（504 PASS / 24 SKIP / 0 FAIL，HEAD `2e248d5a9`，证据 `/tmp/acc-baseline-9b/`），本轮已提交并并入基线。**B2b 流式切片**同样全绿：ctx=262144 全量 529 = 505 PASS / 24 SKIP / 0 FAIL（证据 `/tmp/acc-b2b-262144/`）。**词表修正波复验（§5 #13）**：401/503/exceed 探针 17 PASS / 0 FAIL（`/tmp/env-503-aux/`）；同期 9B 全量已更新为 ctx=262144 三套件 530 = 506 PASS / 24 SKIP / 0 FAIL（`/tmp/best-of-slots/reports-ttlfix/`）。**embeddings live SDK 回归（§8.1 #10）**：9B embeddings 实例（8094）`--only embeddings,sdk` 30 = 29 PASS / 0 FAIL / 1 SKIP（`/tmp/env-emb-8094/`）。**官方接口对齐收束波（§5 #15）**：单模型 9B（8091）三套件 530 = 506 PASS / 24 SKIP / 0 FAIL（与上一基线 519 共有键 **0 状态变化**，仅 ttl 行改名；`/tmp/acc-anth-wave/reports/`）、`--only ws` 41 = 39/2/0、单测 74 passed / 4 deselected，另三项运行期探针（WS 未知 model、echo+logprobs × checkpoint/KV）见 §8.1 #12-#13。
...
| 4 | ~~`/v1/messages` 显式 verbosity/reasoning_effort~~ | **作废（2026-09-14）**：该面已从本 fork 整体移除（§5 #16）。历史结论：官方 Anthropic 请求无顶层 `verbosity`/`reasoning_effort`，故改接官方形态 `output_config.effort` -> `reasoning_effort`、`output_config.format` -> `response_format`、`thinking{disabled}` -> `reasoning_effort="none"`（`ad9e816dd`） | 已移除，不再适用 |
...
2. Responses SSE `obfuscation` 本地偏离 - **已对齐**（2026-09-14 收窄为只给 `*.delta` 事件注入，官方请求侧文案即「delta events」；Chat/Completions 两面的整块注入不变；见 DIFF §4.1 原 #1）；
3. `response.reasoning_text.*` / `response.refusal.*` 不发射；
4. `prompt_cache_options.ttl` 超集 - **已对齐**（外部只收官方 `30m`，另值 400；见 DIFF §4.1 原 #3）；
...
7. Chat `n` 超过 `n_parallel` -> 400（与官方 schema 上限 128 取小）；
8. Chat `logprobs` + `tools` + `stream` 组合 -> 400（官方文档**未禁止**该组合，仅本地限制，不构成违反）；
9. Completions `best_of` 与 `n` - **口径修正**（2026-09-14）：`best_of < n` -> 400，`best_of == n` 合法（官方散文写「greater than」，但 schema 默认 `best_of: 1`、`n` 默认 1，等值必须合法；见 DIFF §4.1 原 #8）；
10. `prompt_cache_diagnostics` 本地简化口径（`reason` 链现已含 `context_compacted`；仍不产出 `unavailable`）；
11. SSE `error` / `response.failed` 本地不可达；
12. Embeddings `encoding_format` 非 string 的 message 透传 - **已对齐**（非 string 与非法字符串都返回官方措辞 400，不再透传 nlohmann 原文；见 DIFF §4.1 原 #11）；
...
26. ~~Anthropic `/v1/messages` 的近似与不生效项~~ - **作废（2026-09-14）**：该面已整体移除（§5 #16），原登记的近似项（`tools[].strict`/`input_examples` 接受即忽略、`usage.service_tier` 恒 `"standard"`、`cache_creation` 恒 0、`inference_geo` 恒 null、`cache_control` 于 `tool_use`/`tool_result` 为消息级近似等）不再适用；历史口径见 `77887e312` / `ad9e816dd`。
27. ~~Anthropic SSE `error` 帧~~ - **作废（2026-09-14）**：该面已整体移除（§5 #16），原「代码已按官方包裹但默认配置运行期不可达」的未验证项随之作废。
...
- 上述 5 条 CI 失败逐条归因与处置（2026-09-14）：
  ...
  4. `unit/test_completion.py::test_n_probs_post_backend_sampling` - **上游遗留行为（非本分支回归），已按套件侧 pin 处置**（2026-09-14）：CI 上稳定 `assert 421 == 354`（两次运行同 id、同 step）。定位链：(a) 本机按 CI 同一构建配置（`-DGGML_CUDA=OFF -DGGML_BLAS=OFF -DGGML_SCHED_NO_REALLOC=ON`）**确定性复现**同一断言与同 id（`/tmp/nprobs-cpu-only.log`）⇒ 与 runner 硬件无关，取决于构建/运行配置；`GGML_SCHED_DEBUG_REALLOC` 开关矩阵否证 NO_REALLOC 相关（build-cpu + `GGML_SCHED_DEBUG_REALLOC=0` 仍 FAIL，CUDA 构建 + `=1` PASS）。(b) **决定性判定**：上游 `origin/master`（`243a3082d`）按同一 CPU-only 配置构建的二进制（`/tmp/llama-up/`）显式 `--top-p 1.0` 后，6 个 seed 的首分歧步与本分支缺省**完全相同**（4/8/12/1/0/2，`/tmp/nprobs-upstream-top1.log`）；同一上游二进制用自身缺省（`top_p=0.95`）则 6/6 一致 ⇒ 上游默认 0.95 截断候选集掩盖了该分歧，本分支按官方对齐把缺省改为 `1.0`（SCOPE「Official parameter defaults」）后暴露。(c) 量化：本分支 CI 配置 6/6 分歧（`/tmp/nprobs-seed-probe.log`），加 `--top-p 0.95` 后 6/6 一致；分歧步两路**报出的分布一致到 ~1e-6**（seed 4245 第 1 步两路均 `ke:0.973860, ked:0.026140`，`/tmp/nprobs-seed-probe2.log`）但选中不同 token ⇒ 分歧在"抽签落在哪个桶"，非概率数值（backend 侧图内 `softmax -> ggml_cumsum -> step` vs legacy 侧 host 累加比较，函数级差异未定位，登记）。V100 构建（CUDA 在场、同 `top_p=1.0`）该行通过，故此前本地基线未见。**处置**：请求内 pin `top_p: 0.95`（`unit/test_completion.py`，断言强度不变、无恒真分支），CI 配置 `1 passed`（4.62 s）、V100 配置 `1 passed`（19.33 s）；登记见 SCOPE「Suite-only fixes」。**上游可复现**：该分歧属上游潜在缺陷（上游自身二进制 + `--top-p 1.0` 即可复现同形分歧），复现材料可交上游维护者（按 fork 纪律，本分支不代为提交 issue/PR）。
...
| Anthropic Messages 面整体移除 | 本轮（维护成本裁定；代码 `925507197`） | `POST /v1/messages` 与 `/v1/messages/count_tokens` 由服务改为 **404**；`X-Api-Key` 认证别名不再被接受（只认 `Authorization: Bearer`）；原先按 Anthropic 形状读取错误体（`{"type":"error",...}`，503 -> 529）或 `usage.cache_creation_*`/`output_tokens_details` 等字段的客户端需改走 Chat Completions / Responses | 该面与其余两面共用引擎入口，维护成本高于收益（用户裁定）；独立切片，可整体 revert（见 §5 #16）；相对上游为永久分叉 |
| 外部 `prompt_cache_options.ttl` 只收 `30m`（`5m`/`1h` -> 400） | 本波（官方接口对齐） | 之前用 `5m`/`1h` 走该字段的客户端现在收 400；`24h` 仍只属于 `prompt_cache_retention` | 官方「only supported value」；独立改动，可单独回退 |
| Responses SSE `obfuscation` 只出现在 `*.delta` 事件 | 本波（官方接口对齐） | 之前依赖「每个 SSE 事件都带 `obfuscation`」做定长填充的客户端，非 delta 事件上不再收到该键；`stream_options.include_obfuscation=false` 语义不变；Chat/Completions 的整块注入不变 | 官方请求侧文案即 delta events、事件 schema 仅 `response.shell_call_command.delta` 声明该字段；独立改动，可单独回退 |
| Completions `best_of < n` -> 400（等值合法） | 本波（官方接口对齐） | 之前 `best_of < n` 也被接受（按 n 生成），现为 400 `'best_of' must be greater than or equal to 'n'`；默认组合 `best_of=1`/`n=1` 不受影响 | 官方散文更严但 schema 默认要求等值合法，取交集口径；独立改动，可单独回退 |
