...
- **Anthropic `/v1/messages` 登记未做项（2026-09-14）** — 官方字段接了合理加深（R1-R7 + S1-S4，见「Anthropic Messages (`/v1/messages`) deepen」），其余登记不修：`thinking{type:"adaptive"}`/`display`、`cache_control` 落 `tools[]`/`tool_use`/`tool_result`、`service_tier`/`container`/`inference_geo`、`tools[].strict`/`input_examples`（本地无对应机制），非流式错误体维持跨端点统一 `{"error":{…}}`（`ex_wrapper`，未按 Anthropic 官方 `{"type":"error","error":{…}}` 改），响应侧 `usage.service_tier`/`stop_details`/`container`/`inference_geo` 与 `message_delta.usage` 的 input/cache_* 仍缺。**已对齐但运行期未触发**：S4 的 SSE error 帧官方包裹（默认配置不可达，超上下文在流开始前即普通 400 JSON 返回）。证据与逐项语义见上节。
...
| `cache_control.ttl` | → `prompt_cache_options.ttl`（本地接受 5m/30m/1h，含官方 5m/1h）。**本地每请求单一 TTL**：多个不同 ttl 折叠为最后一个 | 探针 `ttl=1h accepted` |
...
S4（SSE error 帧官方包裹）在默认配置下**不可达**：探针实测超上下文、长生成、6 并发流式请求均不产生流中 error 帧。超上下文在流开始前以普通 400 JSON 返回 `{"error":{…,"type":"exceed_context_size_error","n_prompt_tokens":1810,"n_ctx":1024}}`（1024 = 每槽可用 ctx）；`max_tokens=2048` 正常收尾；6 并发全部 200（排队而非报错）。可达点在 `server-context.cpp` 两处（流中出现 error 任务 / 流循环抛异常），故记为**代码层对齐、运行期未验证**。证据 `/tmp/anthropic-deepen/probe.log`（15 PASS / 2 FAIL，2 FAIL 均为 S4）。

### 登记未做（官方字段，本地无对应机制）

`thinking{type:"adaptive"}` 与 `display`；`cache_control` 落在 `tools[]` / `tool_use` / `tool_result` 块（本地断点仅支持 text/image part）；`service_tier` / `container` / `inference_geo`；`tools[].strict` / `input_examples`；非流式错误体仍为跨端点统一 `{"error":{…}}`（`ex_wrapper`，路由无关，未按 Anthropic 官方 `{"type":"error","error":{…}}` 改）；`usage.service_tier` / `stop_details` / `container` / `inference_geo`；`message_delta.usage` 仅 `output_tokens`（官方仅要求累计 `output_tokens`）。

### 验收

`unit/test_compat_anthropic.py -m "not slow" -k "not vision"` → **34 passed / 4 deselected**（30.54 s；证据 `/tmp/anthropic-deepen/unit-anthropic-full.log`）；live 探针（V100 + Qwen3.5-9B-Q4_K_M、ctx 2048、2 槽、8095）**15 PASS / 2 FAIL**（2 FAIL = S4 不可触发，见上；证据 `/tmp/anthropic-deepen/probe.log`、`probe-results.json`）。单元套件同时暴露 42 个**预存**失败（非本波引入，见「Suite-only fixes」；2026-09-14 消化：model 校验类已修，余下 16 条按本地缺省 verbosity hint 重基线、10 条离线网络依赖、1 条 metrics 快照差异；重基线后全量离线 11 failed / 343 passed / 11 errors，证据 `/tmp/unit-final.log`）。
