...
- `prompt_cache_ttl_expiry_clears_kv` 的过期后 `cached_tokens=20` 偶发（2026-09-14）：磁盘 TTL 过期只清**绑定该 key 的槽位 KV**（`slot.last_prompt_cache_key == key`）与同 key 的 L2 状态，不清别的槽位；别的槽位（LRU 选中、无 key/隐式 anchor）仍可按**键无关的纯 token LCP** 命中注入的常量 hint 头，于是过期后第 3 次请求报 `cached_tokens=20`（warm 411）而偶发 FAIL。该探针补一条**唯一首条 system 消息**（与本波其它 cache 行同一手法）使 prompt 首 token 唯一，`expired_cached == 0` 断言保持不变（真实回归时第 3 次仍会命中该 key 自己的槽位/L2，`cached ≈ warm` -> 仍 FAIL）。证据：失败现场 `/tmp/best-of-slots/reports-final/responses.json`；同一二进制单 lane 复跑全绿 `/tmp/best-of-slots/ttl-rerun1.json`；修复后整条 responses lane 378 = 356 PASS / 22 SKIP / 0 FAIL（该行 `warm_cached=440 expired_cached=0`，`/tmp/best-of-slots/ttl-fix-run1.json`）与 ctx=262144 全量三套件 530 = 506 PASS / 24 SKIP / 0 FAIL（`/tmp/best-of-slots/reports-ttlfix/`；与首轮现场对拍仅该 2 行 FAIL -> PASS，其余 519 个共有键 0 变化）；`server.log` 两次对照（FAIL：过期清理后第 3 次请求按 LRU 落到无 key 槽位、prompt eval 与冷启动相同 -> cached 20；PASS：落到别的显式 key 槽位、触发 key-change clear -> cached 0）。观察（未改，本地刻意保留的软共享语义）：key 隔离是单向的——别的**显式** key 的槽位不会被复用，无 key/隐式 anchor 的槽位可被任意显式 key 请求复用（套件切片 `af0d0fc63`）
