...
| 7 | Chat `logprobs`+`tools`+`stream` | 官方文档未禁止该组合：`top_logprobs` 只声明「0 到 20」与「`logprobs` 必须为 true」，无 tools/stream 交叉限制（`openai-docs/api/reference/resources/chat.md:1318`）；且明言返回条数可少于请求（`:1461`） | 组合 → 400（本地无法在流式工具调用下稳定产出逐 token logprobs） | 本地限制；官方**无禁止条款**，故不是违反，仅登记 |
| 9 | `prompt_cache_diagnostics` | 云端 token 计量 | 本地简化口径：expected=min(本次/基线 `input_tokens`)，`cached >= expected-4` → `cache_hit`；`reason` 依次回退（命中即止，2026-09-14 起含 `context_compacted`）；`unavailable` 仍不产出 | 详见 scope「Recorded deviations」 |
...
| 15 | WS `/v1/responses` 未知 model | WS error 事件（本地信封） | 400 + `model_not_found`（B 族 message），默认 lane 不带 `stream_id`、不前置事件 | 2026-09-14 起套件已覆盖（`checks_ws.py` 新行 `error shape: unknown model -> model_not_found`，`--only ws` = 41 = 39 PASS / 2 SKIP / 0 FAIL）；实测帧 `/tmp/acc-anth-wave/probe-ws-unknown-model.json`。官方 WS 模式未定义该路径的 status 取值（HTTP 面同请求是 404），本地取 400 保留 |
...
| 20 | Anthropic `/v1/messages` 的近似与不生效项（2026-09-14 全量接线后收窄） | 官方定义 | 仍不生效：`tools[].strict`/`input_examples`（接受即忽略，本地工具语法始终由 schema 驱动）。近似：`service_tier`/`container`/`inference_geo` 请求侧只做类型/枚举校验、不参与调度（本地无 tier/容器/地理概念），响应侧 `usage.service_tier` 恒 `"standard"`、`container`/`stop_details`/`usage.inference_geo` 恒 `null`、`usage.cache_creation.ephemeral_1h_input_tokens`/`ephemeral_5m_input_tokens` 恒 `0`、`usage.server_tool_use` 恒 `{0,0}`（本地 `input_tokens` 已扣缓存命中，报"创建"会破坏官方求和不变式）；`cache_control` 于 `tool_use`/`tool_result` 上落到**消息级**断点（本地无 part 级断点，位置等价、粒度略粗） | 接线细节与逐项契约见 SCOPE「Anthropic Messages」；本行只保留上述近似，其余字段已移出 §4 |
...
本轮已提交（`63fd0490c` + `a9b2ab765` + `2e248d5a9`；本文档随本次提交）；acceptance 三套件已跑全绿（504 PASS / 24 SKIP / 0 FAIL，证据 `/tmp/acc-baseline-9b/`），非流式 `/v1/completions` echo+logprobs 与 `logprobs` 夹取已由探针验证（36/36）。**B2b 流式对偶（原 #17）已实现并验证、随本轮提交**（代码切片 `f27a83df2`）：三套件 505 PASS / 24 SKIP / 0 FAIL（证据 `/tmp/acc-b2b-262144/`）。**Completions `best_of` 容量/官方上限（原 #19 + 新）已实现并验证、随本次提交**（代码切片 `c52034650`）：三套件 530 = 506 PASS / 24 SKIP / 0 FAIL（与 B2b 基线 520 个共有键逐行 0 状态变化、仅 +1 新行；证据 `/tmp/best-of-slots/reports-ttlfix/`）。该切片首轮现场为 530 = 504 PASS / 24 SKIP / 2 FAIL（2 行预存 cache 偶发、非本切片引入），已随同切片套件修复消化（见 §6 尾条）。**2026-09-14 官方接口对齐 wave（本文档随本波提交；代码切片 `ba9b9bbef` + `02fe763e4` + `e93090e88` + `77887e312` + `c219603c9` + `afc6cd4e8` + `891a13277`）**：把 §4 中可在本地落地的项一次收束 —— obfuscation 收窄到 delta 事件（原 #1）、外部 ttl 只收 `30m` + Anthropic ttl 走内部通道（原 #3）、`best_of >= n`（原 #8）、embeddings `encoding_format` 干净 400（原 #11）、Anthropic 非流式错误体改发官方信封与 `max_tokens` 必填（新增两项）；同波还接线了 Anthropic `thinking.adaptive`/`display`、`cache_control` 于 tools/tool_use/tool_result、响应侧 `usage` 细节与 `message_delta.usage`（§4 #20 收窄为近似项），并把 WS 未知 model 从「套件未覆盖」转为套件行。证据见 §6 尾条。
...
| 1 | Responses SSE `obfuscation` 的适用范围（2026-09-14 收窄） | 请求侧文档「adds random characters to an `obfuscation` field on streaming **delta events**」（`openai-docs/api/reference/resources/responses/methods/create.md`）；事件 schema 侧仅 `response.shell_call_command.delta` 声明该字段（同目录 `streaming-events.md`） | 收窄为 `*.delta` 事件才注入（`server_responses_maybe_obfuscate_event` 对非 delta 提前返回）；`stream_options.include_obfuscation=false` 仍整体关闭、replay 的 `include_obfuscation=false` 仍剥离；Chat / legacy Completions 两面的「每个 chunk 都带、末块为 `""`」行为**不变**（`server-task.cpp` 独立注入，官方 chunk schema 确实把该字段定义在整块上） |
| 3 | `prompt_cache_options.ttl`（2026-09-14 收窄） | 官方「defaults to `30m`, which is currently the only supported value」 | 外部只接受 `30m`，其它值 400 `'prompt_cache_options.ttl' must be '30m'`；Anthropic `cache_control.ttl`（`5m`/`1h`）改走内部通道 `__prompt_cache_ttl`（`5m`/`30m`/`1h`），两条入口互不污染 |
| 8 | Completions `best_of` 与 `n` 的关系（2026-09-14 修正） | 文档散文为「`best_of` must be greater than `n`」，但 schema 默认 `best_of: 1` 且 `n` 默认 1（默认组合必须合法 ⇒ 等值合法） | `best_of < n` -> 400 `'best_of' must be greater than or equal to 'n'`；`best_of == n` 正常服务。验收行 `completions_best_of_lt_n_rejected`（`n=2, best_of=1`）与 `completions_best_of_one_suffix_empty_ok`（`best_of=1`，n 缺省 1）双向锁定 |
| 11 | Embeddings `encoding_format` 非 string 的 message | 400 + 官方 message 原文「The format to return the embeddings in. Can be either `float` or [`base64`]…」（`openai-docs/api/reference/resources/embeddings/methods/create.md:51`） | 非 string 与非法字符串（`float`/`base64` 之外）都返回该 message（纯文本，去掉 markdown 链接）+ 400，不再透传 nlohmann `[json.exception.type_error.302]`（`server-context.cpp` embeddings 路由） |
| 新 | Anthropic 非流式错误体（原跨端点统一 -> 官方包裹） | 官方非流式 `{"type":"error","error":{type,message},"request_id"}`；官方「Every API response includes a unique `request-id` header ... The same identifier appears as the `request_id` field in error response bodies」（`anthropic-docs/docs/api/errors.md`） | `is_anthropic_api_path`（`/v1/messages` 与 `/v1/messages/count_tokens`）在 **401 中间件 / 503 中间件 / `ex_wrapper`** 三处改发官方信封；本地 type -> 官方 type/状态码映射见 `anthropic_error_from_local_type`（`server-common.cpp`，如 `service_unavailable_error` -> `overloaded_error`/529）。近似：本地无 per-request id 设施（全仓库无 `request-id` 头），`request_id` 恒 `null`、亦不发 `request-id` 响应头 |
| 新 | Anthropic `max_tokens` 必填（2026-09-14 合规修正） | create 文档 `max_tokens: number`（无 optional 前缀，必填；`0` = 只预热缓存不生成）；`count_tokens` 文档的 body 参数表**没有该字段** | create 缺字段或 null -> 400 `'max_tokens' is required`，非整数或负数 -> 400 `'max_tokens' must be a non-negative integer`；`/v1/messages/count_tokens` 走 `server_chat_convert_anthropic_to_oai(body, /* count_tokens = */ true)`、不要求该字段（`thinking.budget_tokens` 的上界只在与 `max_tokens` 同发时校验） |
...
- 2026-09-14 官方接口对齐收束波（**已提交**：`ba9b9bbef` diagnostics 补 `context_compacted` + `02fe763e4` obfuscation 收窄 + `e93090e88` ttl 只收 `30m` + `77887e312` Anthropic 全量接线 + `c219603c9` embeddings message + `afc6cd4e8` WS 未知 model 套件行 + `891a13277` 偏差注释；本文档随本次提交）：把 §4 中可在本地落地的项一次收束（原 #1/#3/#8/#11 + 新增两项），并关掉两处「已登记未实测」。**acceptance 三套件 530 = 506 PASS / 24 SKIP / 0 FAIL**（responses 378 = 356/22、chat 143 = 142/1、local_durability 9 = 8/1，exit=0；V100 + Qwen3.5-9B-Q4_K_M、单模型 8091；证据 `/tmp/acc-anth-wave/reports/`）；与上一基线 `/tmp/best-of-slots/reports-ttlfix`（去重键 521）对拍：519 个共有键 **0 状态变化**，键差异仅 2 对（两套件的 `prompt_cache_options_ttl_ladder` -> `prompt_cache_options_ttl_only_30m`，修复后均 PASS，`detail: http_30m=200 delta_30m=1800 rejected={'5m': 400, '1h': 400}`）。套件行新增 `checks_ws.py` 的未知 model 行（`--only ws` = 41 = 39 PASS / 2 SKIP / 0 FAIL）。运行期探针：`/v1/messages` 401/400/200 信封、启动窗口 503 -> **529**、WS 未知 model 帧、`echo+logprobs × checkpoint/KV` 组合（4/4 PASS），证据 `/tmp/acc-anth-wave/`。单测 `unit/test_compat_anthropic.py` + `unit/test_security.py`（`-m "not slow" -k "not vision"`，`unshare -n` 离线隔离）= **74 passed / 4 deselected（61.01 s，exit=0）**（`/tmp/acc-anth-wave/unit-anthropic-final.log`）。
- 2026-09-14 Anthropic `/v1/messages` 合理加深（**已提交** `ad9e816dd`；本文档随本次提交）：把官方字段接到本地既有机制 —— 请求侧命名 `tool_choice`（不再退化为 `required`）、`tool_choice{none}`、`disable_parallel_tool_use` -> `parallel_tool_calls=false`、`output_config.effort` -> `reasoning_effort`（官方 5 值子集；越界 400）、`output_config.format` -> `response_format`、`thinking{disabled}` -> `reasoning_effort="none"`（覆盖 effort）、`cache_control` -> 本地显式 `prompt_cache_breakpoint`（含顶层自动缓存落点与 `ttl` -> `prompt_cache_options.ttl`）；响应侧 `usage.cache_creation_input_tokens`（恒 0）、`usage.output_tokens_details.thinking_tokens`、thinking `content_block_start.signature`、SSE `error` 帧官方包裹。**验收**：`unit/test_compat_anthropic.py -m "not slow" -k "not vision"` = **34 passed / 4 deselected**（30.54 s，`/tmp/anthropic-deepen/unit-anthropic-full.log`）+ live 探针（V100 + Qwen3.5-9B-Q4_K_M、8095）**15 PASS / 2 FAIL**（2 FAIL 均为 S4 运行期不可触发，`/tmp/anthropic-deepen/probe.log`、`probe-results.json`）。**同批暴露 42 个预存单测失败**（`unit/test_chat_completion.py` + `unit/test_tool_call.py` = 42 failed / 16 passed / 196 deselected，42.04 s，`unshare -n`；根因 = model 校验 `8cb9a5257` 引入的 `'model' is required` 400 与 `model_not_found` 404，本波 `git diff --stat` 未触碰校验路径；运行期复验见 SCOPE「Suite-only fixes」），属独立波次；2026-09-14 同日消化（model 校验类已修：全量离线 64 failed -> 27 failed；余下 16 条按本地缺省 verbosity hint 重基线、10 条离线网络依赖、1 条 metrics 快照差异；重基线后 11 failed / 343 passed / 11 errors，证据 `/tmp/unit-full-before.log`、`/tmp/unit-fix-verify.log`、`/tmp/unit-final.log`）。契约见 SCOPE「Anthropic Messages (`/v1/messages`) deepen」，差异登记见 §3 与 §4 #20/#21，行为变化面见 PRD §9.1。
