...
| Anthropic Messages 面移除（2026-09-14，本 fork 有意偏离上游） | 本 fork 不再提供 `/v1/messages` 与 `/v1/messages/count_tokens`：转换器、序列化器、Anthropic 错误信封与 type 词表、`X-Api-Key` 认证别名、`__prompt_cache_ttl` 内部通道与 `anthropic_*` 参数全部删除 | 上游 `origin/master` 仍保留该面，故这是对共享文件的**永久分叉**；移除理由为维护成本（该面与 Chat Completions / Responses 共用引擎入口）；端点级证据见 PRD §5 #16 |
...
| 3 | `prompt_cache_options.ttl`（2026-09-14 收窄） | 官方「defaults to `30m`, which is currently the only supported value」 | 外部只接受 `30m`，其它值 400 `'prompt_cache_options.ttl' must be '30m'` |
