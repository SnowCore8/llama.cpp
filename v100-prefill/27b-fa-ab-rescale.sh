#!/usr/bin/env bash
# 微改（rescale skip）：Volta 分支的 VKQ_C 重缩放乘法在 softmax 运行最大值未变化时跳过（KQ_max_scale==1.0f）
# 对照组 e3d = 已提交的 Volta FA 调优配置。协议与 V-A 相同：清场、8k/16k、27B+9B 各两轮。
# 前置: /root/llama.cpp/v100-prefill/fa-libs/{e3d,rescale}/libggml-cuda.so.0.23.0
set -u
...
echo "== fa-ab-rescale 2026-09-12 (清场, 8k/16k)"
